...
- backend/services/health_check/main.py (FastAPI アプリケーション)
- backend/services/health_check/handler.py (Lambda ハンドラー)
...
ディレクトリ構成:
- 本ファイル: tests/backend/services/health_check/test_health_check.py
- テスト対象: backend/services/health_check/main.py, handler.py
"""
...
# テスト対象のインポート
from backend.services.health_check.main import app
...
class TestLambdaIntegration:
    ...
    def test_lambda_handler_import(self):
        ...
        """
        try:
            from backend.services.health_check.handler import handler
