"""
Stripe Payment Succeeded Handler - EventBridge Version

invoice.payment_succeeded イベントを処理する専用Lambda関数
...
async def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    EventBridge から受信したStripe payment_succeeded イベントを処理
    ...
    try:
        ...
        logger.info("Processing payment succeeded event via EventBridge", extra={
            "event_id": event_id,
        ...
        # 処理結果
        result = await process_payment_succeeded(invoice_data, event_id)
        
        logger.info("Payment succeeded event processed successfully", extra={
            "event_id": event_id,
        ...
        return success_response(
            ...
            message="Payment succeeded event processed successfully"
        )
        
    except Exception as e:
        logger.error("Failed to process payment succeeded event", extra={
            "error": str(e),
        ...
        return error_response(
            ...
            message=f"Payment succeeded processing failed: {str(e)}"
        )


async def process_payment_succeeded(invoice_data: Dict[str, Any], event_id: str) -> Dict[str, Any]:
    """
    支払い成功処理のメインロジック（既存webhook_serviceから移植）
    ...
    # 支払い成功ログ記録（既存処理継承）
    period_start = invoice_data.get('period_start', 0)
    subscription_created = subscription.get('created', 0)
    
    if period_start > subscription_created:
        amount_paid_cents = invoice_data.get('amount_paid', 0)
        amount_paid_yen = amount_paid_cents  # JPYの場合は既にyen単位
        
        logger.info("Recurring payment succeeded", extra={
            "user_id": user_id,
            "customer_id": customer_id,
            "amount_paid": amount_paid_yen,
        ...
        logger.info("Initial payment succeeded", extra={
            "user_id": user_id,
            "customer_id": customer_id,
            "event_id": event_id
        })
