...
from homebiyori_common.auth import get_user_id_from_event, require_authentication
from homebiyori_common.logger import get_logger
from homebiyori_common.exceptions import ValidationError, AuthenticationError, DatabaseError, MaintenanceError
from homebiyori_common.maintenance import check_maintenance_mode

# ローカルモジュール
from .models import UserProfile, AIPreferences, ChildInfo, UserProfileUpdate, ChildInfoCreate, ChildInfoUpdate
...
async def maintenance_check_middleware(request: Request, call_next):
    ...
    try:
        ...
        logger.warning("API blocked due to maintenance mode", extra={
        ...
        logger.error("Maintenance check failed, allowing request", extra={
            "error": str(e),
            "request_path": request.url.path
        })
        response = await call_next(request)
...
async def get_user_profile(request: Request):
    ...
    try:
        ...
        if not profile:
            ...
            logger.info("Profile not found, returning default", extra={"user_id": user_id[:8] + "****"})
            profile = UserProfile(user_id=user_id)
        
        return profile
        
    except DatabaseError as e:
        logger.error("Database error in get_user_profile", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
    except Exception as e:
        logger.error("Unexpected error in get_user_profile", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_user_profile(
    ...
    try:
        ...
        if existing_profile:
            # 既存プロフィール更新
            updated_profile = existing_profile.copy(update=profile_update.model_dump(exclude_unset=True))
        else:
            # 新規プロフィール作成
            updated_profile = UserProfile(user_id=user_id, **profile_update.model_dump(exclude_unset=True))
        
        # データベース保存
        saved_profile = await db.save_user_profile(updated_profile)
        
        logger.info("User profile updated successfully", extra={"user_id": user_id[:8] + "****"})
        return saved_profile
        
    except ValidationError as e:
        logger.warning("Validation error in update_user_profile", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=400, detail=str(e))
    except DatabaseError as e:
        logger.error("Database error in update_user_profile", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
    except Exception as e:
        logger.error("Unexpected error in update_user_profile", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_ai_preferences(
    ...
    try:
        logger.info("Updating AI preferences", extra={
            "user_id": user_id[:8] + "****",
        ...
        logger.info("AI preferences updated successfully", extra={"user_id": user_id[:8] + "****"})
        return ai_preferences
        
    except ValidationError as e:
        logger.warning("Validation error in update_ai_preferences", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=400, detail=str(e))
    except DatabaseError as e:
        logger.error("Database error in update_ai_preferences", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_children(request: Request):
    ...
    try:
        ...
        logger.info("Children list retrieved", extra={
            "user_id": user_id[:8] + "****",
        ...
        logger.error("Database error in get_children", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def create_child(
    ...
    try:
        logger.info("Creating new child", extra={
            "user_id": user_id[:8] + "****",
        ...
        logger.info("Child created successfully", extra={
            "user_id": user_id[:8] + "****",
        ...
        logger.warning("Validation error in create_child", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=400, detail=str(e))
    except DatabaseError as e:
        logger.error("Database error in create_child", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****"
        })
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_child(
    ...
    try:
        logger.info("Updating child", extra={
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
        ...
        logger.info("Child updated successfully", extra={
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
        ...
        logger.warning("Validation error in update_child", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
        ...
        logger.error("Database error in update_child", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
...
async def delete_child(child_id: str, request: Request):
    ...
    try:
        logger.info("Deleting child", extra={
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
        ...
        logger.info("Child deleted successfully", extra={
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
        ...
        logger.error("Database error in delete_child", extra={
            "error": str(e),
            "user_id": user_id[:8] + "****",
            "child_id": child_id
        })
