...
- Usersテーブル: USER#{user_id} / PROFILE
- Childrenテーブル: USER#{user_id} / CHILD#{child_id}
- TTL: 無し（永続保存）
...
# ローカルモジュール
from .models import UserProfile, ChildInfo, ChildInfoCreate, ChildInfoUpdate, get_current_utc
...
class UserServiceDatabase:
    ...
    async def get_user_profile(self, user_id: str) -> Optional[UserProfile]:
        ...
        """
        try:
            self.logger.debug("Fetching user profile", extra={
                "user_id": user_id[:8] + "****"
            })
            ...
            if not item_data:
                self.logger.debug("User profile not found", extra={
                    "user_id": user_id[:8] + "****"
                })
                return None
            ...
            self.logger.debug("User profile retrieved successfully", extra={
                "user_id": user_id[:8] + "****",
            ...
            self.logger.error("Failed to get user profile", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****"
            })
    ...
    async def save_user_profile(self, profile: UserProfile) -> UserProfile:
        ...
        """
        try:
            self.logger.debug("Saving user profile", extra={
            ...
            self.logger.debug("User profile saved successfully", extra={
                "user_id": profile.user_id[:8] + "****"
            })
            ...
            self.logger.error("Failed to save user profile", extra={
                "error": str(e),
                "user_id": profile.user_id[:8] + "****"
            })
    ...
    async def get_user_children(self, user_id: str) -> List[ChildInfo]:
        ...
        """
        try:
            self.logger.debug("Fetching user children", extra={
                "user_id": user_id[:8] + "****"
            })
            ...
            for item_data in items:
                try:
                    ...
                    self.logger.warning("Failed to parse child data", extra={
                        "error": str(e),
            ...
            self.logger.debug("User children retrieved successfully", extra={
                "user_id": user_id[:8] + "****",
            ...
            self.logger.error("Failed to get user children", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****"
            })
    ...
    async def create_child(self, user_id: str, child_data: ChildInfoCreate) -> ChildInfo:
        ...
        try:
            ...
            self.logger.debug("Creating new child", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id,
            ...
            self.logger.info("Child created successfully", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id,
                "child_name": child_data.name
            })
            ...
            self.logger.error("Failed to create child", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****",
                "child_name": child_data.name
            })
    ...
    async def get_child(self, user_id: str, child_id: str) -> Optional[ChildInfo]:
        ...
        """
        try:
            self.logger.debug("Fetching child info", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
            ...
            if not item_data:
                self.logger.debug("Child not found", extra={
                    "user_id": user_id[:8] + "****",
                    "child_id": child_id
                })
                return None
            ...
            self.logger.debug("Child info retrieved successfully", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
            ...
            self.logger.error("Failed to get child", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
    ...
    async def update_child(self, user_id: str, child_id: str, child_update: ChildInfoUpdate) -> ChildInfo:
        """
        子供情報更新
        ...
        """
        try:
            self.logger.debug("Updating child info", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id,
                "fields_updated": list(child_update.model_dump(exclude_unset=True).keys())
            })
            ...
            self.logger.info("Child updated successfully", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
            ...
            self.logger.error("Failed to update child", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
    ...
    async def delete_child(self, user_id: str, child_id: str) -> bool:
        ...
        """
        try:
            self.logger.debug("Deleting child", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
            ...
            existing_child = await self.get_child(user_id, child_id)
            if not existing_child:
                self.logger.debug("Child not found for deletion", extra={
                    "user_id": user_id[:8] + "****",
                    "child_id": child_id
                })
                return False
            ...
            self.logger.info("Child deleted successfully", extra={
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
            ...
            self.logger.error("Failed to delete child", extra={
                "error": str(e),
                "user_id": user_id[:8] + "****",
                "child_id": child_id
            })
